...
def convert_shape_format(shape):
    positions = []
    the_shape = shape.shape[shape.rotation % len(shape.shape)]
...
def draw_window(surface, score=0, last_score=0):
    ...
    font = pygame.font.SysFont('comicsans', 30)
    label = font.render('Score: {}'.format(str(score)), 1, WHITE)
    ...
    # last score
    label = font.render('High Score: {}'.format(last_score), 1, WHITE)
...
def main():
    ...
    while run:
        ...
        if fall_time / 1000 > fall_speed:
            fall_time = 0
            current_piece.y += 1
            # to see if it's not the top of the screen
